...
class MultiNavByInput(Node):
    def __init__(self):
        ...
        self.named_goals = {
            ...
            5: [  # Car #2
                {'label': 'Car #2', 'position': {'x': 1.88511, 'y': 0.04972}, 'orientation': {'z': -0.00036, 'w': 0.99999}}
            ],
            6: [  # Pallet P4
    ...
    def request_goal_sequence(self):
        ...
        for num, g_list in self.named_goals.items():
            print(f"{num}: {g_list[-1]['label']}")  # 마지막 단계 이름만 출력
    ...
    def get_result_callback(self, future):
        ...
        self.get_logger().info(f"🎯 {self.goals[self.goal_index]['label']} 도착 완료!\n")
        self.goal_index += 1
        self.send_next_goal()
