...
class MultiNavByInput(Node):
    def __init__(self):
        ...
        self.named_goals = {
            ...
            6: [{'label': 'Pallet P4', 'position': {'x': 0.93976, 'y': 0.22762}, 'orientation': {'z': -0.72112, 'w': 0.69281}}],
            ...
            9: [
                ...
                {'label': 'Pallet P4, P5, P6 Line Home', 'position': {'x': -0.09097, 'y': -0.07632}, 'orientation': {'z': -0.02076, 'w': 0.99978}}
            ],
            10: [
        ...
        self.sub_continue = self.create_subscription(
            Bool, '/nav_continue', self.continue_callback, 10
        )
    ...
    def request_goal_sequence(self):
        ...
        if not self.goals:
            self.get_logger().warn("⚠️ 선택된 목푝이 없습니다.")
            rclpy.shutdown()
            return
    ...
    def get_result_callback(self, future):
        ...
        if label == 'Pallet P4':
            self.get_logger().warn("🚀 Pallet P4 도착 → esp32_full_sequence.py 실행...")
            try:
                subprocess.run([
                    "python3", "/home/pinky/dev_ws/esp32_full_sequence.py",
                    "--case", "2",
                    "--ip", "192.168.0.77",
                    "--port", "8888",
                    "--script", "/home/pinky/dev_ws/aruco_drive.py"
                ])
                self.get_logger().info("✅ ESP32 실행 완료")
            except Exception as e:
                self.get_logger().error(f"❌ ESP32 실행 실패: {e}")

        self.get_logger().info("⏸ /nav_continue 토픽 거래 받을 때까지 모두 정지")
        self.waiting_for_continue = True

    def continue_callback(self, msg):
        if self.waiting_for_continue and msg.data:
            self.waiting_for_continue = False
            self.get_logger().info("▶️ 이동 재가능 신호 수습 → 다음 목푝으로")
            self.goal_index += 1
            self.send_next_goal()
